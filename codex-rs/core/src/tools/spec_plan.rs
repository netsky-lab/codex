...
fn add_core_utility_tools(context: &CoreToolPlanContext<'_>, registry: &mut ToolRegistry) {
    ...
    // Environment-free helper requests (for example recaps) must not gain a local runner.
    if environment_mode.has_environment() && !turn_context.session_source.is_non_root_agent() {
        registry.add(LoopControlHandler);
    }
