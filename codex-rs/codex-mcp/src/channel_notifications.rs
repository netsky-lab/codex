...
// Bound the entire model-visible envelope, including metadata and attachment paths.
// Compact JSON plus its TUI wrapper stays below 8 KiB, even for unusual tokenization.
const MAX_CHANNEL_EVENT_BYTES: usize = 8_000;
...
pub(crate) fn channel_message_event(
    ...
    if serde_json::to_vec(&event).ok()?.len() > MAX_CHANNEL_EVENT_BYTES {
        warn!(server = %server_name, "ignored MCP channel message exceeding the 8000-byte envelope limit");
        return None;
    }
    Some(event)
}
...
mod tests {
    ...
    #[test]
    fn rejects_oversized_text_and_routing_metadata() {
        for params in [
            json!({"text": "x".repeat(MAX_CHANNEL_EVENT_BYTES)}),
            json!({"text": "hello", "metadata": "x".repeat(MAX_CHANNEL_EVENT_BYTES)}),
            json!({"text": "file", "attachments": [{"path": "x".repeat(MAX_CHANNEL_EVENT_BYTES)}]}),
        ] {
            assert!(
                channel_message_event("telegram", "notifications/codex/channel", Some(params))
                    .is_none()
            );
        }
        assert!(
            channel_message_event(
                ...
                Some(json!({"text": "x".repeat(4096), "chat_id": "123"})),
            )
            .is_some()
        );
    }
