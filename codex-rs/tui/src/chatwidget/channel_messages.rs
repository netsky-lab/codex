...
use codex_context_fragments::ChannelMessageContext;
use codex_context_fragments::ContextualUserFragment;
use codex_protocol::models::local_image_label_text;
use codex_protocol::protocol::ChannelMessageAttachment;
use codex_protocol::protocol::ChannelMessageEvent;

pub(crate) fn format_channel_message_for_model(ev: &ChannelMessageEvent) -> Option<String> {
    ChannelMessageContext::new(ev).map(|context| context.render())
}
...
impl ChatWidget {
    pub(crate) fn on_channel_message(&mut self, ev: ChannelMessageEvent) {
        let Some(text) = format_channel_message_for_model(&ev) else {
            tracing::warn!(server = %ev.server, "ignored channel message exceeding the model context limit");
            return;
        };
...
mod tests {
    ...
    fn model_payload_keeps_routing_metadata() {
        let model_text =
            format_channel_message_for_model(&event()).expect("bounded channel message");
